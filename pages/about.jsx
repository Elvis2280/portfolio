...
const About = ({ aboutMe }) => {
  ...
  const controlPicture = useAnimation();
  // if element is in view screen it fire the animation
  const { inView, ref, entry } = useInView();
  if (inView) {
    controlPicture.start({
      opacity: 1,
      transition: {
        duration: 1,
        delay: 0.7,
  ...
  return (
    <>
      <Head>
        <title>About Elvis Miranda</title>
      </Head>
        ...
        <div className="relative w-full pb-96 lg:mb-8">
          ...
          <div ref={ref}>
            <motion.div
              ...
              animate={controlPicture}
              className="absolute block bottom-32 right-14 md:right-0 transform translate-x-2/4 w-44 h-52  md:w-52 md:h-60 lg:w-56 lg:h-64 aboutMe--picBorder"
            >
